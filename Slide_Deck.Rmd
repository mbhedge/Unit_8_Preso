...
adjData %>% 
  ...
  scale_y_continuous(limits = c(0, 0.10),                               # setup the y axis scale, and add 0.62
                     breaks = sort(c(seq(0.25, 0.075, length.out = 3 ),# and 0.04 in it for reference
                                     0.062, 0.04))) +
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.ticks = element_blank(),
        axis.text.y = element_text(family = "Georgia", size = 10),      # set text attributes
        axis.text.x = element_text(family = "Verdana", angle=90, vjust = .02, hjust = 1)) + # rotate the x axis labels 90 degress
    labs(title = "State Median Alcohol By Volume", x="", y="Alcohol By Volume") # +         # set the graph labels
    # coord_fixed(ylim = c(0.025, 0.075))
```
