...
color_boxes <- data.frame(y1=seq(0,40,10), 
                           y2=seq(10,50,10),
...
# style from https://rpubs.com/chidungkt/392980
image <- readJPEG("beer_glass_background.jpg")
bg <- rasterGrob(image, height = unit(1.01, "npc"), x=0.62, y=0.5)
brewery_count_by_state %>% 
  filter(count > 9) %>% 
  mutate(fullname = paste(abbr2state(state), count, sep = " - ")) %>% 
  ggplot() + 
  annotation_custom(bg, -Inf, Inf, -Inf, Inf) + # beer background
  geom_bar(aes(x=reorder(fullname, count), y = count),stat="identity", 
           fill = "brown", 
           color = "gold",
           alpha = 0.9) + 
  geom_rect(data=color_boxes,
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.text.y = element_text(family = "Georgia", size = 10)) +
  labs(title = "States with 10 or More Breweries", x="", y="Number of Breweries") + 
  coord_flip()
```
...
```{r east_coast_breweries, include=FALSE}
...
To allow us to continue without removing any of the records for missing values, it was decided to impute the data with a k-NN on beers in the same "Style".  What this means is that, for a given beer, the algorithm will look at the beers in the same style, determine an appropriate IBU or ABV value, and replace the missing value in the sample.

Another way to fill in the missing values in this case, would be to commission additional research and attempt to fill in the missing values by looking up the ABV/IBU for each beer.  This could be accomplished by looking at the beer's website, advertising materials, or calling the brewery and asking.
...
adjData %>% group_by(State) %>% summarize(Median=median(ABV)) %>% ggplot(aes(x=State, y=Median)) +
  geom_bar(stat="identity") +
  coord_flip()
```

```{r median IBU per state}
adjData %>% group_by(State) %>% summarize(Median=median(IBU)) %>% 
...
adjData %>% group_by(State) %>% summarize(Max=max(IBU)) %>% 
  ggplot(aes(x=State, y=Max)) +
  geom_bar(stat="identity") +
  coord_flip()
```
```{r highest ABV per State }
adjData %>% group_by(State) %>% summarize(Max=max(ABV)) %>% 
  ggplot(aes(x=State, y=Max)) +
  geom_bar(stat="identity") +
  coord_flip() +
