...
brewery_count_by_state %>% 
  ...
  geom_bar(aes(x=reorder(fullname, count), y = count),stat="identity", 
           ...
           color = "gold",
           alpha = 0.9) + 
  geom_rect(data=color_boxes, mapping=aes(ymin = y1, ymax = y2, xmin = -Inf, xmax = +Inf), fill = color_boxes$color) +
  # geom_text(aes(x=fullname, y = count), label=count ) +
  theme_minimal() +
  ...
  labs(title = "Breweries per State", x="", y="Number of Breweries") 
```
...
adjData %>% 
  ...
  ggplot() +
    annotation_custom(bg, -Inf, Inf, -Inf, Inf) + # beer background
    geom_bar(aes(x=reorder(State, medabv), y=medabv),stat="identity", color="gold", fill="brown") +
    geom_rect(data=color_boxes,
           mapping=aes(ymin = y1, ymax = y2, xmin = -Inf, xmax = +Inf),
           fill = color_boxes$color) +
    geom_text(aes(x=reorder(State, medabv), y=medabv, label=format(round(medabv,3), nsmall = 3)), size = 3, angle=90, hjust = -.4) +
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.ticks = element_blank(),
        axis.text.y = element_text(family = "Georgia", size = 10),
        axis.text.x = element_text(family = "Verdana", angle=90, vjust = .02, hjust = 1)) +
