...
bg <- rasterGrob(image, height = unit(1.5, "npc"), x=0.62, y=0.5)
...
brewery_count_by_state %>% 
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.text.x = element_text(family = "Verdana", angle=90, hjust = 1)) +
...
```{r map_the_breweries, include=FALSE} 
...
image <- readJPEG("beer_glass_background.jpg")
# scale the background for this chart
bg <- rasterGrob(image, height = unit(1, "npc"), x=0.5, y=0.5)
...
adjData %>% 
  group_by(State) %>% 
  summarize(medabv=median(ABV)) %>%
  mutate(State = abbr2state(str_trim(as.character(State)))) %>% # turn 2 letter factor into full state name
  mutate(State = paste(State, round(medabv, 3), sep = " - ")) %>% # append the median ABV to the state name 
  ggplot() +
    annotation_custom(bg, -Inf, Inf, -Inf, Inf) + # beer background
    geom_bar(aes(x=reorder(State, medabv), y=medabv),stat="identity") +
    geom_rect(data=color_boxes,
           mapping=aes(ymin = y1, ymax = y2, xmin = -Inf, xmax = +Inf),
           fill = color_boxes$color) +
    theme_minimal() +
   # geom_text(aes(x=reorder(State, medabv), y=medabv),size=2, hjust=-1, color="red", label = medabv) +
    theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.ticks = element_blank(),
        axis.text.y = element_text(family = "Georgia", size = 10)) +
    labs(title = "State Median Alcohol By Volume", x="", y="Alcohol By Volume") + 
